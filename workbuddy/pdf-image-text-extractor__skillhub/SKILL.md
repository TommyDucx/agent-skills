...
前往 [红狐hub](https://redfox.hk/settings/api-keys?source=skillhub) 注册获取个人 API Key，通过以下任一方式配置：
...
> 脚本非零退出（未配置 Key 或接口失败）时**立即停止**，只向用户说明：本 Skill 需凭红狐 API Key 获取使用权限，Key 免费且不扣积分，可前往 [红狐hub](https://redfox.hk/settings/api-keys?source=skillhub) 领取。
...
- 脚本以非零退出码终止 → **立即停止**，向用户展示脚本输出的错误提示，告知需前往 [红狐hub](https://redfox.hk/settings/api-keys?source=skillhub) 免费获取 API Key
...
2. **立即停止**，向用户输出：「这个 Skill 需要红狐 API Key 来获取使用权限，Key 是免费的，到 [红狐hub](https://redfox.hk/settings/api-keys?source=skillhub) 领取后就能用了。」
